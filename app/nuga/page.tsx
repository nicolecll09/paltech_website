"use client";

import Container from "@/components/ui/Container";
import { motion } from "motion/react";
import Navbar from "@/components/layout/Navbar";
import Footer from "@/components/layout/Footer";
import CoverProject from "@/components/layout/CoverProject";
import { ImageSlide } from "@/components/layout/CoverProject";
import { LogoItem } from "@/components/layout/Logos";
import Logos from "@/components/layout/Logos";


const slides: ImageSlide[] = [
    {
      id: 1,
      src: "/images/cover/cow.JPG",
      title: "NUGA",
      subtitle:
        "EIP-Agri",
    },
    {
      id: 2,
      src: "/images/cover/berg.jpg",
      title: "NUGA",
      subtitle:
        "EIP-Agri",
    },
  ];

  const logos: LogoItem[] = [
  { id: 1, src: "/images/logos/projects/1.jpg", alt: "Partner logo 1" },
  { id: 2, src: "/images/logos/projects/2.png", alt: "Partner logo 2" },
  { id: 3, src: "/images/logos/projects/3.png", alt: "Partner logo 3" },
  { id: 4, src: "/images/logos/projects/4.png", alt: "Partner logo 4" },
  { id: 5, src: "/images/logos/projects/5.png", alt: "Partner logo 5" },
  { id: 6, src: "/images/logos/projects/6.png", alt: "Partner logo 6" },
  { id: 7, src: "/images/logos/projects/7.png", alt: "Partner logo 7" },
  { id: 8, src: "/images/logos/projects/8.jpg", alt: "Partner logo 8" },
];

function SectionDivider() {
  return (
    <div className="my-8 h-px w-24 bg-[#7ea36a]/40 md:my-10 md:w-32" />
  );
}

function SectionTitle({
  eyebrow,
  title,
}: {
  eyebrow?: string;
  title: string;
}) {
  return (
    <div>
      {/* {eyebrow && (
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#6e8d5a]">
          {eyebrow}
        </p>
      )} */}
      <h2 className="mt-3 font-[var(--font-heading)] text-3xl font-semibold tracking-[-0.03em] text-[#102018] md:text-5xl">
        {title}
      </h2>
    </div>
  );
}

function SectionTitleWhite({
    eyebrow,
    title,
  }: {
    eyebrow?: string;
    title: string;
  }) {
    return (
      <div>
        {eyebrow && (
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-white">
            {eyebrow}
          </p>
        )}
        <h2 className="mt-3 font-[var(--font-heading)] text-3xl font-semibold tracking-[-0.03em] text-white md:text-5xl">
          {title}
        </h2>
      </div>
    );
  }

function BulletList({ items }: { items: string[] }) {
  return (
    <div className="mt-6 space-y-4">
      {items.map((item) => (
        <div key={item} className="flex items-start gap-4">
          <div className="mt-2 h-2.5 w-2.5 rounded-full bg-[#6e8d5a]" />
          <p className="text-base leading-7 text-[#102018]/78 md:text-lg md:leading-8">
            {item}
          </p>
        </div>
      ))}
    </div>
  );
}

export default function NugaPage() {
  return (
    <>
    <Navbar />
    <main className="bg-[#f7f8f4] text-[#102018]">
    <CoverProject slides={slides}/>

      {/* HERO */}
      <section className="relative overflow-hidden border-b border-[#102018]/8 bg-white py-24 md:py-32">
        <Container>
            
          <div className="mx-auto max-w-7xl">
            <h1 className="mt-4 font-[var(--font-heading)] text-4xl font-semibold leading-[1.02] tracking-[-0.04em] text-[#102018] md:text-6xl xl:text-7xl">
                  Europäische Innovationspartnerschaft (EIP-Agri)
            </h1>
            <motion.div
              initial={{ opacity: 0, y: 24 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.7 }}
              className="grid items-center gap-12 lg:grid-cols-[0.5fr_0.5fr]"
            >
              <div>




                <p className="mt-16 max-w-3xl text-lg leading-8 text-[#102018]/72 md:text-2xl text-justify">
                    Die Europäische Innovationspartnerschaft für landwirtschaftliche Produktivität und Nachhaltigkeit (EIP-Agri) ist ein Förderinstrument der EU. Es hat das Ziel, Innovationen im Agrarsektor zu stärken. Gefördert werden praxisorientierte Projekte, die Landwirtschaft und Ernährung produktiver und gleichzeitig nachhaltiger gestalten.<br/><br/>
                    Unser Projekt NUGA wird im Rahmen von EIP-Agri durch die Europäische Union und den Freistaat Bayern finanziell unterstützt und hat eine Laufzeit von 24 Monaten.
                </p>
              </div>



                <div className="mt-6  gap-4">
                    <img
                      src="/images/nuga/nuga.jpg"
                      alt="NUGA"
                      className="">
    
                    </img>
                    <div className="mt-6 flex justify-center gap-4 bg-[#506c35] p-4 rounded-2xl text-white text-center">
                        Download Erläuterungstafel (PDF)
                    </div>
                </div>


            </motion.div>
          </div>
        </Container>
      </section>

      {/* EIP-AGRI */}
      <section className="py-16 md:py-20 bg-white">
        <Container>
          <motion.div
            initial={{ opacity: 0, y: 18 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.35 }}
            transition={{ duration: 0.6 }}
            className="mx-auto max-w-7xl rounded-[28px] border border-[#6e8d5a]/18 bg-[#506c35] px-8 py-10 text-white shadow-[0_20px_60px_rgba(110,141,90,0.18)] md:px-12"
          >
            <p className="text-4xl font-semibold text-center uppercase tracking-[0.22em] text-white">
                Ziel des Projekts
            </p>

            <p className="mt-16 text-xl text-center font-medium leading-8 md:text-2xl md:leading-9">
            Ziel des Projekts NUGA ist die Entwicklung eines innovativen
                Anbaugeräts für Schlepper, das eine mechanische und damit
                herbizidfreie Bekämpfung von Ampfer und Kreuzkraut im Grünland
                ermöglicht und somit die Futterqualität erhöht.
            </p>

            {/* <p className="mt-6 text-base leading-7 text-white/85 md:text-lg">
              Unser Projekt NUGA wird im Rahmen von EIP-Agri durch die
              Europäische Union und den Freistaat Bayern finanziell unterstützt
              und hat eine Laufzeit von 24 Monaten.
            </p> */}

            {/* <a
              href="/pdfs/erlaeuterungstafel.pdf"
              className="mt-6 inline-flex items-center rounded-full bg-white px-6 py-3 text-sm font-semibold transition hover:bg-white/90"
            >
            <p className="text-black">Download Erläuterungstafel (PDF)</p>
          
            </a> */}

          </motion.div>
          <Logos logos={logos} />
        </Container>
      </section>

      {/* ZIEL DES PROJEKTS */}


      {/* PROBLEM */}
        <section className="border-y border-[#102018]/6 py-20 md:py-28">
            <Container>
                <div className="mx-auto max-w-7xl">
                <div className="grid items-start gap-14 lg:grid-cols-[1fr_1fr] lg:gap-20">
                    {/* LEFT: TEXT */}
                    <motion.div
                    initial={{ opacity: 0, y: 22 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true, amount: 0.25 }}
                    transition={{ duration: 0.6 }}
                    className="max-w-xl"
                    >
                    <SectionTitle
                        eyebrow="02"
                        title="Warum Ampfer und Kreuzkraut ein Problem sind"
                    />
                    <SectionDivider />

                    <p className="text-base leading-7 text-black md:text-lg md:leading-8">
                        Ampfer gilt als Platzräuber: Er verdrängt wertvolle
                        Futterpflanzen, bildet tiefe Pfahlwurzeln und produziert eine
                        große Menge langlebiger Samen. Die mechanische Entfernung ist
                        schwierig.
                    </p>

                    <p className="mt-6 text-base leading-7 text-black md:text-lg md:leading-8">
                        Kreuzkraut ist giftig für Tiere, insbesondere für Rinder und
                        Pferde. Selbst im Heu bleibt die Toxizität bestehen.
                    </p>

                    <p className="mt-6 text-base leading-7 text-black md:text-lg md:leading-8">
                        Im ökologischen Landbau dürfen keine chemischen Mittel eingesetzt
                        werden – es bleibt bislang nur das händische Stechen. Dieses
                        benötigt etwa eine Minute pro Pflanze – ein immenser Zeitaufwand,
                        der in Zeiten von Arbeitskräftemangel kaum zu leisten ist.
                    </p>
                    </motion.div>

                    {/* RIGHT: IMAGE */}
                    <motion.div
                    initial={{ opacity: 0, y: 28 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true, amount: 0.2 }}
                    transition={{ duration: 0.65 }}
                    className="relative"
                    >
                    <div className="overflow-hidden rounded-[30px] border border-[#102018]/8 bg-white shadow-[0_25px_70px_rgba(16,32,24,0.08)]">
                        <img
                        src="/images/nuga/nuga_3.jpg"
                        alt="Ampfer und Kreuzkraut im Grünland"
                        className="h-full w-full object-cover"
                        />
                    </div>
                    </motion.div>
                </div>
                </div>
            </Container>
        </section>



      <section className="py-20 md:py-28 bg-white">
        <Container>
            <div className="mx-auto max-w-7xl">
            <div className="grid items-start gap-14 lg:grid-cols-[0.95fr_1.05fr] lg:gap-20">
                <motion.div
                initial={{ opacity: 0, y: 22 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.25 }}
                transition={{ duration: 0.6 }}
                >
                <SectionTitle eyebrow="03" title="Bisherige Ergebnisse" />
                <SectionDivider />

                <BulletList
                    items={[
                    "Es wurde ein digitaler Zwilling des Anbaugeräts erstellt.",
                    "Verschiedene Methoden zur Optimierung der Einsatzplanung wurden getestet.",
                    "Die Risikoanalyse wurde erfolgreich abgeschlossen.",
                    "Nun beginnt die physische Umsetzung des Anbaugeräts – ein entscheidender Schritt zur Praxisreife.",
                    ]}
                />
                </motion.div>

                <motion.div
                initial={{ opacity: 0, y: 28 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.65 }}
                className="grid grid-cols-1 gap-5 sm:grid-cols-1"
                >
                <div className="overflow-hidden rounded-[5px] bg-white">
                    <img
                    src="/images/nuga/nuga_1.png"
                    alt="Digitaler Zwilling des Anbaugeräts"
                    className="h-full w-full object-cover"
                    />
                </div>

                <div className="overflow-hidden rounded-[5px]  bg-white">
                    <img
                    src="/images/nuga/nuga_2.png"
                    alt="Optimierung der Einsatzplanung"
                    className="h-full w-full object-cover"
                    />
                </div>


                </motion.div>

            </div>
            </div>
        </Container>
        </section>


      {/* CTA PDF */}
      <section className="bg-[#102018] py-20 text-white md:py-28">
        <Container>
          <div className="mx-auto max-w-4xl text-center">
            <motion.div
              initial={{ opacity: 0, y: 22 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.25 }}
              transition={{ duration: 0.6 }}
            >
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#b8d2a8]">
                Weitere Informationen
              </p>

              <h2 className="mt-4 font-[var(--font-heading)] text-3xl font-semibold tracking-[-0.03em] md:text-5xl">
                Erläuterungstafel herunterladen
              </h2>

              <p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-white/72 md:text-lg">
                Laden Sie die Projektunterlagen herunter, um weitere Details zu
                Förderung, Zielen und Umsetzung von NUGA zu erhalten.
              </p>

              <a
                href="/pdfs/erlaeuterungstafel.pdf"
                className="mt-8 inline-flex items-center rounded-full bg-white px-7 py-3 text-sm font-semibold text-[#102018] transition hover:bg-white/90"
              >
                <p className="text-black">PDF herunterladen</p>
          
              </a>
            </motion.div>
          </div>
        </Container>
      </section>

      <Footer />
    </main>
    </>
  );
}